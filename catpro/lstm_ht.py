...
def trainLSTM(X_train=None, y_train=None, X_dev=None, y_dev=None, R_train=None, R_dev=None, hyperparams=None,
    ...
    if layertype == 'lstm':
        if nlayers == 1:
            ...
            #                       trainable=True, mask_zero=True))
            # model.add(LSTM(hsize, activation=activation_function, return_sequences=False, batch_input_shape = (batch_size, timesteps, dim), recurrent_dropout=dropout_rec, dropout=dropout))
            model.add(LSTM(hsize, activation=activation_function, return_sequences=False, input_shape=(timesteps, dim),
                           recurrent_dropout=dropout_rec, dropout=dropout))
            # model.add(Dense(10, activation='relu'))
            # model.add(Lambda(lambda x: x, output_shape=lambda s: s, mask=None, arguments=None)) #TODO: added, mask?
            # model.add(Flatten())
        ...
        if nlayers == 2:
            model.add(LSTM(hsize, activation=activation_function, return_sequences=True, input_shape=(timesteps, dim),
                           recurrent_dropout=dropout_rec, dropout=dropout))
            model.add(
                LSTM(hsize, activation=activation_function, return_sequences=False, recurrent_dropout=dropout_rec))
            # model.add(Dense(dsize, activation=act_output))
        ...
        if nlayers == 4:
            model.add(LSTM(hsize, activation=activation_function, return_sequences=True, input_shape=(timesteps, dim),
                           recurrent_dropout=dropout_rec, dropout=dropout))
            model.add(
                LSTM(hsize, activation=activation_function, return_sequences=True, recurrent_dropout=dropout_rec, ))
            model.add(LSTM(hsize, activation=activation_function, return_sequences=True, recurrent_dropout=dropout_rec))
            model.add(
                LSTM(hsize, activation=activation_function, return_sequences=False, recurrent_dropout=dropout_rec))
            # model.add(Dense(dsize, activation=act_output))
    ...
    if call_back:
        ...
        # log best model
        checkpoint_best = ModelCheckpoint(filepath_best, monitor='val_loss', verbose=0, save_best_only=True,
                                          mode='auto')

        # log improved model
        checkpoint_epochs = ModelCheckpoint(filepath_epochs, monitor='val_loss', verbose=0, save_best_only=True,
                                            mode='auto')
...
def combineFeats():
    ...
    hyperparams = {'exp': 20, 'timesteps': 30, 'stride': 1, 'lr': 9.9999999999999995e-07, 'nlayers': 3, 'hsize': 128,
                   ...
                   'dim': 100, 'min_count': 3, 'window': 3, 'wepochs': 25, 'layertype': 'bi-lstm', 'merge_mode': 'mul',
                   'path_to_dir': 'data/LSTM_10-audio/', 'exppath': 'data/LSTM_10-audio/20/',
    ...
    hyperparams = {'exp': 330, 'timesteps': 7, 'stride': 3, 'lr': 0.10000000000000001, 'nlayers': 2, 'hsize': 4,
                   ...
                   'dropout': 0.10000000000000001, 'dropout_rec': 0.80000000000000004, 'loss': 'binary_crossentropy',
                   'dim': 100, 'min_count': 3, 'window': 3, 'wepochs': 25, 'layertype': 'bi-lstm',
                   'merge_mode': 'concat', 'path_to_dir': 'data/LSTM_10/', 'exppath': 'data/LSTM_10/330/',
                   'text': 'data/Step10/alltext.txt', 'balClass': False}
    exppath = hyperparams['exppath']
...
def load_train_data_lstm(dataset='train', group_by='response', run_audio=config.config['run_audio'],
...
if __name__ == "__main__":
    ...
    if group_by == 'interview':
        ...
        X_train_text, X_train_audio, y_train_all = load_train_data_lstm(dataset='train',
                                                                        group_by='response',
                                                                        use_doc2vec=True)  # TODO, use put parameters in config
    ...
    hyperparams_lstm = {'timesteps': timesteps, 'activation_function': ['relu', 'tanh'], 'act_output': ['sigmoid'],
                        'nlayers': nlayers, 'hsize': hsize,
                        'batchsize': [64, 128], 'epochs': epochs, 'dropout': [0.2, 0.4, 0.6],
    ...
    hyperparams_bilstm = {'timesteps': timesteps, 'activation_function': ['relu', 'tanh'], 'act_output': ['sigmoid'],
                          ...
                          'balClass': [True, False], 'optimizer': ['Adam', sgd], 'dropout_rec': dropout_rec,
                          'merge_mode': ['mul', 'concat']}
    hyperparams_final = {'timesteps': [33], 'activation_function': ['relu'], 'act_output': ['sigmoid'], 'nlayers': [2],
                         'hsize': [128],
    ...
    for i, gridsearch_i in enumerate(gridsearch):
        ...
        X_train_audio_i = pad_sequences(X_train_audio, maxlen=timesteps_i, padding=padding,
                                        dtype='float32')  # Not specifying dtype results in wrong data
